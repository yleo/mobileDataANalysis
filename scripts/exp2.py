        ...
        if (u_previous == u_current):
            ...
            for day, hour in user_calls:
                if (len(user_calls[(day, hour)])>=2 and len(user_bs[(day, hour)])>=1 and len(user_calls_only[(day, hour)])>0):
                    ...
                    output.write(str(day)+'\t'+str(hour)+'\t'+str(user_calls_only[(day, hour)][random_call_only])+'\t'+str(len(user_calls_only[(day, hour)]))+'\t'+str(user_calls[(day, hour)][random_call])+'\t'+str(len(user_calls[(day, hour)]))+'\t'+str(user_bs[(day, hour)][random_bs])+'\t'+str(len(user_bs[(day, hour)]))+'\t'+str(user_duration[(day, hour)][random_call])+'\n')
            user_calls = {}
            user_duration = {}
            user_bs = {}
